...
adminApp.constant('FBURL', 'https://dev-nurha.firebaseio.com');
adminApp.constant('APPURL', 'https://stage-nurha.firebaseapp.com/');

adminApp.config(['$urlRouterProvider', '$logProvider', '$mdThemingProvider', 'cmsClientProvider', 'FBURL',
function($urlRouterProvider, $logProvider, $mdThemingProvider, cmsClientProvider, FBURL) {
  $logProvider.debugEnabled(true);
  $urlRouterProvider.otherwise('/');
  cmsClientProvider.setContentUrl(FBURL+'/app/content');
      ...
      var Schema = function(item) {
        if(item.parent === 'events'){
          return {
            ...
            caption: item.dateString
          }
        } else if(item.parent === 'halls'){
          return {
            title: item.title,
            subtitle: item.locationString,
            caption: item.representative.name + '  ' +item.representative.email
          }
        } else if(item.parent === 'people'){
          return {
            title: item.name,
            subtitle: item.email,
            caption: item.title
  ...
  return {
    ...
    controller: function ($scope) {
      $scope.editItem = function (evt) {
        ...
        $scope.edit(evt, $scope.item.parent, $scope.item.key);
      };
    },
    transclude: true,
...
adminModule.config(['$stateProvider', function($stateProvider) {
  $stateProvider
    ...
    .state('admin.messages', {
      ...
      views: {
        'admin-content': {
          templateUrl: 'modules/admin/templates/admin-messages-index.html'
        }
      },
      data: {
        title: 'Messages'
...
adminModule.controller('AdminController', [
  ...
  $scope.create = function(ev, objectType){
    $scope.$content.focusedData = null;
    showDialog(ev, 'modules/admin/templates/dialog-new-event.html', 'push', '/app/content/'+objectType);
  };

  $scope.edit = function(ev, objectType, objectKey){
    // TODO set focused data here
    $scope.$content.focusedData = $scope.$content.events[objectKey];
    showDialog(ev, 'modules/admin/templates/dialog-new-event.html', 'update', '/app/content/'+objectType);
  };
...
homeModule.controller('HomeController',['$log', '$scope', '$mdToast', 'FBURL', 'APPURL', function ($log, $scope, $mdToast, FBURL, APPURL){
  ...
  var messageRef = messagesList.push();
  ...
  var showToast = function(status){if(status==='success'){$mdToast.show(toasts.success);} else {$mdToast.show(toasts.error);}};
  ...
  $scope.submit = function(){
    if ($scope.contact.$valid) {
      ...
      $scope.messageData.submitAtString = Date(Firebase.ServerValue.TIMESTAMP);
      messageRef.set($scope.messageData, function(error){
        if(error){
